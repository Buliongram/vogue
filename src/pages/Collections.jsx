...
export default function Collections() {
  const collectionName = window.location.pathname.replace("/collections/", "");
  const [showFilters, setShowFilters] = useState(false);

  const cartLength = clothing.filter(
    (cloth) => cloth.section1 || cloth.section1 === "men"
  ).length;

  const doesPathExist = clothing.filter(
    (cloth) => cloth.section === collectionName
  );
  const pathexsist = doesPathExist.length;
        ...
        <>
          <header className="relative h-[100px] md:h-[120px] lg:h-[170px] overflow-hidden w-full flex items-center justify-center  opacity">
            <img
              ...
              src={clothing
                .filter((el) => el.sectionImg === collectionName)
                .map((el) => el.image)}
              alt="banner"
            />
              ...
              <section className="w-full">
                ...
                <main className="w-full mt-5">
                  <div className="newArrivals">
                    ...
                    {ascending === 0
                      ? clothing
                          .filter(
                            (cloth) =>
                              cloth.section1 || cloth.section == collectionName
                          )
                          .map((cloth) => (
                            <Products key={cloth.id} {...cloth} />
                          ))
                      : ascending === 1
                      ? clothing
                          .filter(
                  ...
                  <div className="text-sm mt-10">
                    ...
                    accessories to all luxury connoisseurs including Men, Women,
                    and Kids.
                  </div>
                </main>
              </section>
