...
export default function CreateAddress({ handleLogOut }) {
  ...
  const handleAddressUpdate = async (e) => {
    ...
    try {
      const sendRequest = await fetch(
        "http://localhost:3000/address/createAddress",
        {
          headers: {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        ...
        // Update address inputs with user data once fetched
        setAddressInputs((prev) => ({
          ...prev,
    ...
    <>
      <section className="lg:p-12 py-8 flex flex-col md:flex-row gap-6">
        ...
        >
          ...
          <section className="p-4 items-center justify-center w-full mx-auto flex flex-col gap-6">
