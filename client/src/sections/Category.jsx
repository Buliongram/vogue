...
export default function Category() {
  return (
    ...
    >
      {clothing
        .filter((clothing) => clothing.category === true)
        .map((category) => (
          <SwiperSlide key={category.id}>
            <div className="relative items-center cursor-pointer  overflow-hidden group flex flex-col gap-2 font-semibold">
              ...
              <Link
                key={category.id}
                to={category.path}
                className="border border-gray-500 hover:border-transparent text-center font-medium py-1 px-2 md:py-2 md:px-4 hover:text-white rounded-md uppercase text-xs md:text-sm hover:bg-gray-900 w-max"
              >
                DISCOVER MORE
              </Link>
            </div>
